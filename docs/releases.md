...
Real package testing is a manual gate: on native Intel and Apple Silicon machines with MacPorts, test a normal port and representative architecture-specific fallbacks (MAS, rtk/worktrunk, and signal-cli/OpenJDK). Review privileged operations first. Record the macOS version, architecture, MacPorts version, commands, results, and known limitations in the release notes. Mocked CI does not establish successful real package installation.
